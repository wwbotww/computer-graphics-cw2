...
{
	Mat44f ret = kIdentity44f;
	ret[0,3] = aTranslation.x;
	ret[1,3] = aTranslation.y;
	ret[2,3] = aTranslation.z;
	return ret;
}
inline
...
{
	auto const tanHalfFov = std::tan( aFovInRadians * 0.5f );
	Mat44f ret{};

	ret[0,0] = 1.f / ( aAspect * tanHalfFov );
	ret[1,1] = 1.f / tanHalfFov;

	auto const invDepth = 1.f / ( aNear - aFar );
	ret[2,2] = ( aFar + aNear ) * invDepth;
	ret[2,3] = ( 2.f * aFar * aNear ) * invDepth;
	ret[3,2] = -1.f;
