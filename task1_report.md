...
### 视觉设定
- 采用灰度环境光/漫反射（ambient `vec3(0.25)`、diffuse `vec3(0.75)`）与方向光 `(0,1,-1)`，`glClearColor` 也改为浅灰以匹配文档示例 Figure 1。
- 当前渲染输出为灰度版的 n·l shading，与作业要求一致，后续可在同一框架内加入纹理贴图（Figure 2）。
...
## 截图与结果
- 灰度 n·l 地形视角（与 Figure 1 类似）可参见 `doc/screenshots/task1_2_gray_1.png`（需运行程序后自行截取至少三张不同视角用于报告）。
- Catch2 测试通过截图/日志已保存在终端输出（见上述命令）。

## 后续工作
- Task 2 及以后可在现有框架下添加 OBJ 纹理读取、PBR/多光源、UI 叠加等；摄像机与数学库已经满足进一步扩展需求。
